...
async function getJohnDoe() {
  try {
    console.log('🔍 Finding John Doe patient...');
    
    const { data: patient, error } = await supabase
      .from('users')
      .select('id, name, email')
      .eq('email', 'john.doe@sospatient.com')
      .single();

    if (error) {
      throw new Error(`Error fetching John Doe: ${error.message}`);
    }

    if (!patient) {
      throw new Error('John Doe not found in database');
    }

    console.log(`✅ Found patient: ${patient.name} (${patient.email}) - ID: ${patient.id}`);
    return patient;
  } catch (error) {
    throw new Error(`Exception fetching John Doe: ${error.message}`);
  }
}
...
// Create appointments for John Doe with various doctors
async function createAppointmentsForJohnDoe() {
  console.log('🏥 SOS Tourist Doctor - Creating Appointments for John Doe');
  console.log('='.repeat(60));
  
  try {
    // Get John Doe patient
    const patient = await getJohnDoe();
    ...
    console.log('\n📋 Creating 30 appointments for John Doe...');
    ...
    console.log(`✅ Successfully created ${data.length} appointments for John Doe!`);
...
createAppointmentsForJohnDoe().then(appointments => {
  if (appointments && appointments.length > 0) {
    console.log(`\n🎉 Successfully created ${appointments.length} appointments for John Doe!`);
