...
async function distributeInvoicesWithNewStatuses() {
  console.log('📊 Distributing invoices with new statuses...');
  console.log('='.repeat(45));
  
  try {
    // Test if new statuses are available
    console.log('\n🔍 Testing if new statuses are available...');
    ...
    for (const status of testStatuses) {
      ...
      if (error) {
        console.log(`   ❌ "${status}" - Not available:`, error.message);
      } else {
    ...
    if (workingStatuses.length === 0) {
      console.log('\n❌ New statuses are not available yet. Please wait a moment and try again.');
      return;
    }
    
    console.log('\n✅ New statuses are available:', workingStatuses);
...
distributeInvoicesWithNewStatuses();